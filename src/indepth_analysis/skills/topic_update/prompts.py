...
- 날짜·시간(UTC 또는 현지) 명기
- **시점 정합성**: 모든 사건·수치에 발생/발표일(YYYY-MM-DD)을 부착. {cutoff} 이전이라도 **전년도 이전 사건을 최근·현재 일로 오인 금지** — 반복형 사건(무역합의·정상회담·금리결정·선거)은 발생 연도를 반드시 명시
...
- URL 필수
- 발표 시간 ± 타임존
- **시점 정합성**: 성명·발표에 발생일(YYYY-MM-DD) 부착. 전년도 이전 성명을 현재 발표처럼 서술 금지 — 발생 연도 명시
...
- 출처 등급 A/B/C 표기
- 개인 신상·미확인 정보는 skip
- **시점 정합성**: 각 포스트의 timestamp(YYYY-MM-DD) 부착. 과거 포스트를 최근 발언처럼 인용 금지 — 게시 연도 명시
...
5. **시점 정합성(중요)**: KCIF PDF는 과거 발간분이 섞여 있다. 각 PDF의 **발간일**을 명시하고, 그 안의 사건을 인용할 때 **원 사건 발생일을 보존**하라. 오래된 PDF의 과거 사건을 현재 사건처럼 서술 금지 — 발간일과 사건일을 구분 표기
...
- **시점 정합성**: 모든 가격에 관측일(YYYY-MM-DD) 부착. 과거 시점 가격을 현재가로 오인 금지 — 연도 다른 데이터 혼입 주의
...
- **시의성**: {cutoff} 이전 정보만
- **시점 정합성(필수)**: 모든 사건에 발생 연도·일자를 명시하고, **전년도(또는 그 이전) 사건을 당해 연도 사건으로 치환 금지**. baseline·KCIF 등 과거 자료에서 인용 시 원 사건일을 보존하고, 반복형 사건(무역합의·정상회담·금리결정·선거)은 연도를 명확히 구분
- 소셜미디어 인용은 원문 + 한국어 병기
...
### 1b. 실시간 시장 데이터 독립 검증 (HIGH — Gatherer 일치만으로 PASS 금지)
Gatherer가 틀린 숫자를 가져왔을 때 Draft도 같은 오류를 공유하므로,
**내부 일치 ≠ 외부 정확성**. 아래 항목은 반드시 WebSearch로 독립 재확인 후 판정:
- **주가·YTD 수익률**: Yahoo Finance / 해당 거래소 공식 사이트에서 직접 조회.
  YTD(연초 대비)와 52주 고점-저점 낙폭(peak-to-trough)을 혼동하는 패턴 특히 주의.
- **시가총액·EV**: 현재가 × 발행주식 수로 역산 교차 확인.
- **분기·연간 실적**: 공식 IR 자료 또는 Bloomberg/Reuters 1차 보도 교차 검증.
  Gatherer가 수집한 IR 컨퍼런스 수치와 공식 공시 수치의 차이 확인.
- **시장 규모·CAGR 추정치**: 인용 리서치 리포트 원출처가 실제로 해당 수치를 제시하는지 확인.
독립 검증 없이 "gatherer_c와 일치 → PASS" 판정을 내리는 것은 이 항목에서 HIGH 위반이다.

### 2. 시의성·시점 정합성 (HIGH — 시대착오 능동 탐지)
- {cutoff} 이후 데이터 혼입 여부 / 가장 오래된 claim 날짜
- **연도 오인(anachronism) 검증**: 날짜 있는 주요 주장에 대해 "이 사건이 정말 분석 기간({window_start}~{cutoff})의 것인가"를 검증. **전년도(또는 그 이전) 사건을 당해 연도 사건으로 오인**하지 않았는지 독립 WebSearch로 확인. 반복형 사건(무역합의·정상회담·금리결정·선거)의 **연도 치환 오류**를 최우선 탐지 — 예: "2025년 미·중 합의"를 "2026년"으로 기술.
- **stale-as-current 검증**: 출처 published_date가 분석 기간보다 현저히 과거인데 현재형으로 서술된 항목 flag.
- 사전 등록된 교정 항목뿐 아니라 **신규 시대착오도 능동 탐색**. 발견 시 HIGH로 분류하고 정확한 연도/일자를 제시.
