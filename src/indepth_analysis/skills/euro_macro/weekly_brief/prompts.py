...
- **시점 정합성**: 관세·정책 등 이벤트에는 발생일(YYYY-MM-DD)을 부착하고, 전년도 이전 사건을 최근 일로 오인하지 말 것. 반복형 사건(무역합의·금리결정)은 발생 연도 명시.
"""
...
텍스트와 차트 JSON을 다음 8가지 기준으로 평가하고, 수정해야 할 이슈를 \
...
8. **시점 정합성(시대착오)**: 날짜 있는 사건·수치가 보고 시점과 정합한가. 전년도(또는 그 이전) 사건을 당해 연도로 오인하지 않았는가(반복형 사건의 연도 치환 오류 — 예: "2025년 합의"를 "2026년"으로). 연도 불일치 발견 시 HIGH로 분류.
